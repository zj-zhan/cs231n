...
def sample_noise(batch_size, dim, seed=None):
    ...
    return torch.rand(batch_size, dim) * 2 - 1 #要求噪声区间在（-1,1），但是torch只能生成（0,1）
...
def bce_loss(input, target):
    ...
    # return bce(input.squeeze(), target)  会导致input的size变成（N,）而target的size变成（N,1）,故修改成下面的return
...
def get_optimizer(model):
    ...
    #Make a function that returns an optim.Adam optimizer for the given model with 
